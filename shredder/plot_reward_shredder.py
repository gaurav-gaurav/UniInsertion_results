...
td3 = pd.read_csv('TD3_InsertPaper_V-v0_Rewards.csv')
sac = pd.read_csv('SAC_InsertPaper_V-v0_Rewards.csv')
dreamer = pd.read_csv('Dreamer_InsertPaper_V-v0_Rewards.csv')

td3 = np.array(td3['scores'])
sac = np.array(sac['scores'])
dreamer = np.array(dreamer['scores'])
td3 = td3.reshape(-1, 5).mean(axis = 1)
