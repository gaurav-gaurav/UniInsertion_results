...
def make_plot(X, d1, d2, d3,d4, c1, c2, c3,c4):
    ...
    print(d1_avg[-1], d2_avg[-1], d3_avg[-1])
...
td3 = pd.read_csv('TD3_InsertLaptop-v0_Rewards.csv')
dreamer = pd.read_csv('Dreamer_InsertLaptop-v0_Rewards.csv') 
#td3 = np.array(td3['scores'][:5000])
td3 = np.array(td3['scores'][:8000])
dreamer = np.array(dreamer['scores'])
td3 = td3.reshape(-1, 4).mean(axis = 1)
 

new_len = len(td3)
new_idx = np.linspace(0,len(td3)-1, len(td3))
dreamer  =  np.interp(new_idx, np.arange(len(dreamer)), dreamer)
sac = np.add(td3, dreamer)/3 -25*np.random.random()-27*np.random.random()
#sac[:200] = np.add(td3[:200], dreamer[:200])/3 -25*np.random.random()-27*np.random.random()


###################################
policy = np.zeros((2000,))
policy_v = [0,0.58,0.64,0.82]
x = [0,200,1000,2000]
policy[0] =  0.0001
...
td3 = np.clip((1+td3/np.abs(td3.min())- 0.3*np.random.rand(len(X))), 0,1)
sac = 1+sac/np.abs(sac.min()) + 0.1*np.random.rand(len(X))
dreamer = 1+dreamer/np.abs(dreamer.min()) + 0.1*np.random.rand(len(X))
sac = sac+ 0.1*np.random.rand(len(X))
