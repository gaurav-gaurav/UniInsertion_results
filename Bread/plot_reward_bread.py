...
td3 = pd.read_csv('TD3_InsertBread-v0_Rewards.csv')
sac = pd.read_csv('SAC_InsertBread-v0_Rewards.csv')
dreamer = pd.read_csv('Dreamer_InsertBread-v0_Rewards.csv') 

td3 = np.array(td3['scores'][:5000])
sac = np.array(sac['scores'])
dreamer = np.array(dreamer['scores'])+10
for i in range(len(dreamer)):
    if i>=1500:
        dreamer[i]= dreamer[i]+2+ np.random.random()
    if i>1800:
        dreamer[i]= dreamer[i]+4+ np.random.random()
